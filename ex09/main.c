...
MODULE_DESCRIPTION("My /proc/mounts list");

static struct proc_dir_entry *proc_entry;

static void traverse_mounts_tree(struct mount *mnt, struct seq_file *m)
{
    struct mount *child;
    struct path path;
...
static int mounts_show(struct seq_file *m, void *v)
{
    ...
    traverse_mounts_tree(ns->root, m);
